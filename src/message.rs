    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(18) => msg.serial.push(r.read_string(bytes).map(Cow::Borrowed)?),
...
impl<'a> MessageWrite for ChannelMessage<'a> {
    fn get_size(&self) -> usize {
        ...
        + self.serial.iter().map(|s| 1 + sizeof_len((s).len())).sum::<usize>()
    }

    fn write_message<W: Write>(&self, w: &mut Writer<W>) -> Result<()> {
        w.write_packed_with_tag(10, &self.ids, |w, m| w.write_int32(*m), &|m| sizeof_varint(*(m) as u64))?;
        for s in &self.serial { w.write_with_tag(18, |w| w.write_string(&**s))?; }
        Ok(())
    }
}
